...
async function convert(targetDirectory, sourceDirectory) {
...
async function createFolderStructure(targetDirectory) {
...
function initializeBasicImageMetadata(features, allImagesMetadata, fileName) {
    var _a;
    const dateTaken = (_a = features['Profile-EXIF']) === null || _a === void 0 ? void 0 : _a['Date Time Original'];
    return allImagesMetadata.concat({ name: fileName, date: dateTaken, resolutions: {} });
}
async function identifyAndConvertImage(filePath, fileName, targetDirectory, allImagesMetadata) {
...
async function createVariousImageResolutions(filePath, fileName, targetDirectory, overallResolve, overallReject, allImagesMetadata) {
...
async function provideImageInformation(webPath, allImagesMetadata, targetDirectory) {
...
async function saveMetadataFile(sortedMetadataArray, targetDirectory) {
...
async function sortByCreationDate(imageMetadata) {
...
async function sortByFileName(imageMetadata) {
...
async function sortByFileNameReverse(imageMetadata) {
    console.log('\nSorting images by file name reversed...');
    imageMetadata.sort((a, b) => a.name.localeCompare(b.name));
    console.log('...done.');
    console.log('eoeoeo');
    console.log(imageMetadata[0].name);
    const reversed = [...imageMetadata].reverse();
    console.log(reversed[0].name);
    return reversed;
}

async function doNothing(imageMetadata) {
    console.log('\nSorting images by file name...');
    imageMetadata.sort((a, b) => a.name.localeCompare(b.name)).reverse();
    console.log('...done.');
    return imageMetadata;
}

async function sortByPrimaryColor(imageMetadata) {
...
async function determineSortFunction(argv) {
    switch (true) {
        case argv['d']:
        ...
        case argv['r']:
            await logInfo('Going to reverse sort images by file name.');
            return sortByFileNameReverse;
        case argv['c']:
            await logInfo('Going to sort images by color (experimental).');
            return sortByPrimaryColor;
        default:
            await logInfo('No sorting mechanism specified! Do nothing.');
            return doNothing;
    }
}
await execute();
